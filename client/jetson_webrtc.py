...
webrtcbin name=sendrecv bundle-policy=max-bundle \
 ...
 ! nvv4l2h264enc control-rate=1 bitrate=1000000 peak-bitrate=2000000  cabac-entropy-coding=true insert-sps-pps=true profile=4  ! video/x-h264,  stream-format=(string)byte-stream \
 ! rtph264pay config-interval=-1 \
 ! queue ! application/x-rtp,media=video,encoding-name=H264,payload=97 ! sendrecv. \
 alsasrc device=plughw:CARD=Device,DEV=0  ! audioconvert ! audioresample ! queue ! opusenc ! rtpopuspay ! \
        ...
        async for message in self.conn:
            if message.startswith('ONLINE:'):
                if not self.start_connection:
                    self.peer_id = int(message.split(':')[1])
                    print ('new peer online %s' % self.peer_id)
                    time.sleep(2)
